'' @script EnviaAfip 3.4   31-07-2020
...
Public AlicuotaTributo,	BaseImponibleTributo,DescripcionTributo,ImporteTributo,aToken,aSign,nro_calipso_Actual,id_Solicitud,ExisteSolicitud,sCbu,sAlias,_
	   nContribuyente,imp_Tolerancia,cPesos,cDolares,cEuros,cReales,xValid,xSintecrom,xZSA,xCabal,xEgramar,xProdalsa,xBinova,xWSMTXCA,xUtilizaPathInstalacionSA,CAE
				...
				' Obtiene la posición de IVA.	
					...
					Else
						...
						If (oPosicion.Coeficiente = 0) Then
							id_0 = 3
				'			imp_tot_conc = imp_tot_conc +  ObtenerTotalItem (oItemComprobante,esdelgada)
						...
						ElseIf (oPosicion.Coeficiente = 10.5) Then
							id_105 = 4
							If xValid Then	
								base_imp_105 = Round(base_imp_105 +  ObtenerTotalItem (oItemComprobante,esdelgada) / (1 + oPosicion.Coeficiente) ,4)
								valoriva_105 = Round(valoriva_105 + round(ObtenerTotalItem (oItemComprobante,esdelgada) / ((1+oPosicion.Coeficiente)/oPosicion.Coeficiente)),4)
						...
						ElseIf (oPosicion.Coeficiente = 21) Then
							id_21 = 5
							If xValid Then	
								base_imp_21 = Round(base_imp_21 +  ObtenerTotalItem (oItemComprobante,esdelgada) / (1 + oPosicion.Coeficiente) ,4)
								 valoriva_21 = Round(valoriva_21 + round(ObtenerTotalItem (oItemComprobante,esdelgada) / ((1+oPosicion.Coeficiente)/oPosicion.Coeficiente)),4)
								TotalNetoGravado = TotalNetoGravado +  round((ObtenerTotalItem (oItemComprobante,esdelgada) / (1+oPosicion.Coeficiente)),4)
							else
						...
						ElseIf (oPosicion.Coeficiente = 27) Then
							...
							If xValid Then	
								...
								valoriva_27 = Round(valoriva_27 + round(ObtenerTotalItem (oItemComprobante,esdelgada) / ((1+oPosicion.Coeficiente)/oPosicion.Coeficiente)),4)
								TotalNetoGravado = TotalNetoGravado +  round((ObtenerTotalItem (oItemComprobante,esdelgada) / (1+oPosicion.Coeficiente)),4)
							else
...
'*******************************************************************************
				...
				ElseIf (tipo_cbte = 3 OR tipo_cbte = 2) and Not pComprobante.BOExtension.MiPyMEs Then
stop
				   'set xFacturaVinculada = pComprobante.VinculoTr
                   set xComprobanteAsociado = nothing
                   If xProdalsa or xZSA Then
                   ...
                   Else
                      If xProdalsa or xZSA Then
                         fecha_desde_DocAsoc = pComprobante.boextension.AFIP_PERIODO_DESDE 
                         fecha_hasta_DocAsoc = pComprobante.boextension.AFIP_PERIODO_HASTA
...
'------ Modificado por Diego Boero 3/9/2019
									'If oImpuesto.Coeficiente > 0 Then
									If oImpuesto.Importe > 0 Then
										'AlicuotaTributo 		 	= oImpuesto.Coeficiente
										'BaseImponibleTributo 		= Round((Round(oImpuesto.Importe,2) * 100) / oImpuesto.Coeficiente,2)
...
'------ Fin Modificado por Diego Boero 3/9/2019
...
'------ Modificado por Diego Boero 3/9/2019
									...
									If oImpuesto.valor.Importe > 0 Then
										'AlicuotaTributo 		= oImpuesto.Coeficiente
										'BaseImponibleTributo 	= Round((Round(oImpuesto.valor.Importe,2) * 100) / oImpuesto.Coeficiente,2)
